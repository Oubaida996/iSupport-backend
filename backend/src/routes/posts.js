...
router.get("/posts/author", bearerAuth, aclAuth('read'), getPostsFromUserHandler);
router.get("/posts/community/:id", bearerAuth, aclAuth('read'), getPostsFromCommunityHandler);
...
async function updatePostInfoHandler(req, res) {
  ...
  if (toUpdate) {
    if (toUpdate.author === req.user.id) {
      ...
      res.status(204).send(`You can't edite this post because you aren't the author `);
    }
  } else {
    res.status(500).send(`the  post id ${pid} isn\'t exist`)
  }
...
async function deletePostHandler(req, res) {
  ...
  if (fetchedPost) {
    if (fetchedPost.author === req.user.id) {
      ...
      res.status(204).send(`You can't delete this post because you aren't the author `);
    }
  } else {
    res.status(500).send(`the  post id ${pid} isn\'t exist`)
  }
...
async function getPostsFromUserHandler(req, res) {
  ...
  let fetchedPosts = await database.posts.findAll({ where: { author: req.user.id }, include: [database.users, database.communities] });
  if (fetchedPosts) {
...
async function getPostsFromCommunityHandler(req, res) {
  ...
  let fetchedPost = await database.posts.findAll({ where: { community_id: cid }, include: [database.users] });
  if (fetchedPost) {
    res.status(200).json(fetchedPost);
  } else {
